...
operation = 0;
model_ind = 1;
phi_sqrt_std_std = NaN;

[lh, phi_sqrt] = likelihood_func(Cd, rhoa_obs_log, phs_obs, f_rhoa, f_phs, rho_log, z_log, k_weight, scale_factor);
...
model_cell{model_ind, 7} = phi_sqrt; % L2范数误差
phi_sqrt_mat = [phi_sqrt; zeros(N_end-1, 1)]; % L2范数误差向量
phi_sqrt_std_mean_mat = zeros(N_end/N_refresh, 2); % 误差标准差及平均值
refresh_ind = 0;
...
while model_ind < N
    ...
    if mod(model_ind, 2) == 0
        ...
        [lh, phi_sqrt] = likelihood_func(Cd, rhoa_obs_log, phs_obs, f_rhoa, f_phs, rho_log_new, z_log_new, k_weight, scale_factor);
    elseif flag < 0.34
        n_rnd = randi(n_new-1);
...
%     prior_probability_new = 1;
    ...
    if rand < ppd_new/ppd && length(z_log_new) == length(unique(z_log_new)) && isequal(z_log_new, sort(z_log_new)) && max(rho_log_new) < rho_mesh(end)
        ...
        model_cell{model_ind, 7} = phi_sqrt;
        rho_log = rho_log_new;
        z_log = z_log_new;
        ppd = ppd_new;
        n = n_new;
        ...
        phi_sqrt_mat = [phi_sqrt_mat(2:end); phi_sqrt];
...
%             person_corr_min = min(min(person_corr));
            ...
            % 判定误差的变化幅度
            phi_sqrt_std = std(phi_sqrt_mat);
            phi_sqrt_mean = mean(phi_sqrt_mat);
%             disp([num2str(phi_sqrt_std), '  ', num2str(phi_sqrt_mean)]);
            
            % 判定误差分布的变化幅度
            phi_sqrt_std_mean_mat = [phi_sqrt_std_mean_mat(2:end, :); [phi_sqrt_std, phi_sqrt_mean]];
            if model_ind >= N_end
                phi_sqrt_std_mean_std = std(phi_sqrt_std_mean_mat);
                phi_sqrt_std_std = phi_sqrt_std_mean_std(1); % 取标准差为判定指标
            end
            ...
            if plot_flag == 1
                ...
                annotation('textbox',[.9 0 .1 .3], 'String','误差的标准差：','EdgeColor','none');
                annotation('textbox',[.9 0 .1 .2], 'String',num2str(phi_sqrt_std_std),'EdgeColor','none');
            ...
            disp(['迭代数：', num2str(model_ind), '，PPD峰值RMS误差：', num2str(rms_max), '，PPD期望RMS误差：', num2str(rms_average), '，过去', num2str(N_end), '次迭代的误差标准差：', num2str(phi_sqrt_std_std)]);
            
            % 判断模型是否已稳定
            if model_ind > N_end
                if phi_sqrt_std_std < std_target
                    t = toc(t_func);
                    ...
                    end_flag = 1;
                    break;
%                 elseif phi_sqrt_std_std < std_target * 10
%                     burn_in_flag = 2;
%                     model_grid = zeros(mesh_layers_n, mesh_rho_n); % 重新初始化后验概率密度矩阵
%                     scale_factor = scale_factor_origin;
                end
            end
...
%             end
        ...
        if cnt_rejections > N_refresh && model_ind == 1
            end_flag = 2;
            break
        end
    end
