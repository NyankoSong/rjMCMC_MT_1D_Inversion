function [model_cell, model_grid, end_flag, model_ind] = TransD(rho_mesh, z_mesh, f_rhoa, f_phs, rhoa_obs_log, rhoa_obs_err_log, phs_obs, phs_obs_err, N, N_refresh, rms_target, std_target, N_end, k_punish, f_Cd, k_weight, k_err, k_err_rhoa, k_err_phs, k_smooth, z_smooth_log, m_test, z_test)
% 单马尔科夫链程序——可变维
% end_flag % 迭代结束时是否达标
...
prior_probability = exp(-norm((Cm^-(1/2))*rho_log * k_smooth)^2/2); % 先验（仅包含平滑）
...
while model_ind < N
    ...
    if flag < 0.33 % 扰动电阻率 （TODO:考虑将操作概率转变为变量）
        ...
        lh = likelihood_func(Cd, rhoa_obs_log, phs_obs, f_rhoa, f_phs, rho_log_new, z_log_new, k_weight, k_err_vec);
    elseif flag < 0.67 % 扰动分层位置
        ...
        if (rand < 0.5 || n_new >= n_range(2)) && n_new > n_range(1) % 灭
            ...
            operation = 3;
            n_new = n_new - 1;
            lh = likelihood_func(Cd, rhoa_obs_log, phs_obs, f_rhoa, f_phs, rho_log_new, z_log_new, k_weight, k_err_vec) * (2-k_punish);
        else % 生，使第一层和最后一层可以生
...
%                 z_log_new = [mesh_func(rand * z_mesh_log(n_rnd), z_mesh_log); z_log_new(n_rnd:end)];
            ...
            operation = 4;
            n_new = n_new + 1;
            lh = likelihood_func(Cd, rhoa_obs_log, phs_obs, f_rhoa, f_phs, rho_log_new, z_log_new, k_weight, k_err_vec) * k_punish;
        end
    end
...
%     prior_probability_new = exp(-norm((Cm_new^(-1/2))*((rho_log_new-mean(rho_log_new))/std(rho_log_new)))^2/2);
    prior_probability = exp(-norm((Cm_new^(-1/2))*rho_log_new*k_smooth)^2/2);
...
%         lh = lh_new;
        ...
        if mod(model_ind, N_refresh) == 0
            ...
            [model_average_response_rho_log, ~] = forward_func(model_average_log, z_mesh_log, f_rhoa);
            [~, model_average_response_phs] = forward_func(model_average_log, z_mesh_log, f_phs);
            [~, model_max_log_ind] = max(model_grid, [], 2);
            model_max_log = rho_mesh_log(model_max_log_ind);
            [model_max_response_rho_log, ~] = forward_func(model_max_log, z_mesh_log, f_rhoa);
            [~, model_max_response_phs] = forward_func(model_max_log, z_mesh_log, f_phs);
            ...
            [rho_max_log, ~] = forward_func(model_max_log, z_mesh_log, f_rhoa);
            [~, phs_max] = forward_func(model_max_log, z_mesh_log, f_phs);
            [rho_average_log, ~] = forward_func(model_average_log, z_mesh_log, f_rhoa);
            [~, phs_average] = forward_func(model_average_log, z_mesh_log, f_phs);
            model_average = 10.^model_average_log;
            model_max = 10.^model_max_log;
            rms_max = rms(([rhoa_obs_log; phs_obs] - [rho_max_log; phs_max])./[rhoa_obs_err_log; phs_obs_err]);
            rms_average = rms(([rhoa_obs_log; phs_obs] - [rho_average_log; phs_average])./[rhoa_obs_err_log; phs_obs_err]);
            ...
            if plot_flag == 1
                ...
                semilogy(model_average_response_rho_log, f_rhoa, 'LineWidth', 1)
                axis([log10(rho_mesh(1)), log10(rho_mesh(end)), f_rhoa(1), f_rhoa(end)]);
                grid on
                hold on
                semilogy(model_max_response_rho_log, f_rhoa, 'r', 'LineWidth', 1)
                ...
                subplot(1, 4, 4); % 正演相位
                semilogy(model_average_response_phs, f_phs, 'LineWidth', 1)
                axis([0, 90, f_phs(1), f_phs(end)]);
                grid on
                hold on
                semilogy(model_max_response_phs, f_phs, 'r', 'LineWidth', 1)
