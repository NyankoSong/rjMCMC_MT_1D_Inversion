...
grid on
loglog(rho_median, z_mesh, 'g', 'LineWidth', 1)
if test_flag == 1
    plot([rho_test(1), rho_test(1)], [z_mesh(1), z_test(1)], 'k--', 'LineWidth', 1)
    stairs(rho_test, z_test, 'k--', 'LineWidth', 1);
    plot([rho_test(end), rho_test(end)], [z_test(end-1), z_mesh(end)], 'k--', 'LineWidth', 1);
    
    legend('99%置信区间', '95%置信区间', '68%置信区间', 'PPD中位数', '测试模型')
else
...
bar(model_n_hist(:, 2));
hold on
xline(peaks_loc(1), 'k--', 'LineWidth', 1)
grid on
axis([3-model_n_hist(1), 30-model_n_hist(1), 0, 0.3])
title('模型维数概率密度柱状图')
xlabel('模型层数')
ylabel('概率')
